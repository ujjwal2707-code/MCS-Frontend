...
class AdsServicesModule(private val reactContext: ReactApplicationContext) :
    ...
    fun getInstalledApps(promise: Promise) {
        try {
            ...
            for (appInfo in apps) {
                // Only include apps that have a launch intent (i.e., are visible to the user)
                ...
                // Retrieve permissions if available.
                try {
    ...
    @ReactMethod
    fun getAdsServices(promise: Promise) {
        try {
            val pm = reactContext.packageManager
            // Retrieve all installed packages including their services.
            val packages = pm.getInstalledPackages(PackageManager.GET_SERVICES)
            val adsServicesArray: WritableArray = Arguments.createArray()

            // Loop through each package to search for ad-related services.
            for (pkg in packages) {
                pkg.services?.forEach { service ->
                    // Filter based on service name (here, checking if it contains "ad").
                    if (service.name.contains("ad", ignoreCase = true)) {
                        val serviceMap: WritableMap = Arguments.createMap()
                        serviceMap.putString("packageName", pkg.packageName)
                        serviceMap.putString("serviceName", service.name)
                        adsServicesArray.pushMap(serviceMap)
                    }
                }
            }
            promise.resolve(adsServicesArray)
        } catch (e: Exception) {
            promise.reject("ERROR", e)
        }
    }
